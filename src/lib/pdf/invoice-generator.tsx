...
const getStatusColor = (status: string): { backgroundColor: string; color: string } => {
  ...
  return colors[status.toLowerCase()] || { backgroundColor: '#F3F4F6', color: '#4B5563' };
};
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={[styles.row, { marginTop: 30 }]}>
          <View style={styles.section}>
            ...
            <Text style={styles.value}>{order.shipping_address?.name || 'N/A'}</Text>
            <Text style={styles.value}>{order.email}</Text>
          </View>
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Ship To:</Text>
            <Text style={styles.value}>{order.shipping_address?.line1 || 'N/A'}</Text>
            ...
            <Text style={styles.value}>{order.shipping_address?.country || 'N/A'}</Text>
          </View>
        </View>
        ...
        <View style={styles.table}>
          ...
          {order.order_items.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              ...
              <Text style={styles.col2}>{formatCurrency(item.price, order.currency)}</Text>
              <Text style={styles.col3}>{item.quantity}</Text>
              <Text style={styles.col4}>{formatCurrency(item.subtotal, order.currency)}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={[styles.section, { marginTop: 30 }]}>
          <Text style={styles.label}>Payment Status: <Text style={styles.value}>{order.payment_status}</Text></Text>
        </View>
